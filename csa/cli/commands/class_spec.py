...
def class_spec_command(
    ...
    set_command_context("command-name")
    logger = get_logger(__name__, command="command-name")
    ...
    try:
        with pool.connection() as conn:
            ...
            logger.info("=" * 80)
            logger.info("Class Specification Generator")
            logger.info("=" * 80)
            logger.info(f"Project: {project_name}")
            logger.info(f"Class: {class_name}")
            logger.info(f"Output Directory: {output_dir}")
            logger.info(f"Include CRUD Info: {include_crud_info}")
            logger.info("")
            ...
            if gen_result["success"]:
                ...
                logger.info("")
                logger.info(f"File saved to: {gen_result['file_path']}")
                logger.info("")
                logger.info("Summary:")
                stats = gen_result["stats"]
                logger.info(f"  - Class Name: {stats.get('class_name', 'N/A')}")
                logger.info(f"  - Package: {stats.get('package_name', 'N/A')}")
                logger.info(f"  - Type: {stats.get('type', 'N/A')}")
                logger.info(f"  - Sub Type: {stats.get('sub_type', 'N/A')}")
                logger.info(f"  - Methods: {stats.get('methods_count', 0)}")
                logger.info(f"  - Fields: {stats.get('fields_count', 0)}")
                logger.info(f"  - Dependencies: {stats.get('dependencies_count', 0)}")
                logger.info(f"  - Tables Used: {stats.get('tables_count', 0)}")
                ...
                logger.info("")
                logger.info(f"✗ Failed to generate specification: {gen_result['message']}")
