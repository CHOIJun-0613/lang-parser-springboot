...
class Parser(object):
    ...
    def __init__(self, tokens):
        ...
        self.debug = False
        self._in_switch_expression_label = False
...
# -- Switch and for statements ---
    ...
    def parse_switch_expression(self):
        ...
        cases = list()
        while not self.try_accept('}'):
            labels = self.parse_switch_expression_labels()
            self.accept('->')
            ...
            if self.would_accept('{'):
                ...
                result_expression = self.parse_expression()
                self.accept(';')
                statements = None
                is_block = False

            case_node = tree.SwitchExpressionCase(case=labels,
                                                  result=result_expression,
                                                  statements=statements,
                                                  is_block=is_block)
            cases.append(case_node)

        return tree.SwitchExpression(expression=selector, cases=cases)

    @parse_debug
    def parse_switch_expression_labels(self):
        if self.try_accept('default'):
            return ['default']

        self.accept('case')
        self._in_switch_expression_label = True
        try:
...
# -- Expressions --
    ...
    def parse_expressionl(self):
        ...
        if not self._in_switch_expression_label and self.would_accept('->'):
