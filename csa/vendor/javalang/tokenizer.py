...
class JavaTokenizer(object):
    ...
    def read_string(self):
        if self.data.startswith('"""', self.i):
    ...
    def read_text_block(self):
        ...
        while True:
            if j >= length:
                self.error('Unterminated text block literal')
                break

            if self.data.startswith('"""', j):
                backslash_count = 0
                k = j - 1
                while k >= self.i + 3 and self.data[k] == '\\':
                    backslash_count += 1
                    k -= 1
